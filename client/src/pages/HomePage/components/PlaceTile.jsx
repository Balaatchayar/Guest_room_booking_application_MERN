...
const PlaceTile = ({ place }) => {
    const [redirect, setRedirect] = useState(); //  to handle redirection

    const handleRedirect = () => {
        setRedirect("/places/" + place._id); //  redirect path based on place ID
    };

    if (redirect) {
        return <Navigate to={redirect} />; // Redirect to the specified path if `redirect` state is set
    }

    return (
        <div
            onClick={handleRedirect} // Handle click event to trigger 
